...
def route_tab(flights):
	...
	def make_dataset(origin, destination):
		...
		subset = flights[(flights['Continent'] == destination) & (flights['Position Group'] == origin)]
	...
	def make_plot(src, origin, destination, label_dict):
		...
		p = figure(plot_width = 800, plot_height = 600, x_axis_label = 'Overall Rating', y_axis_label = '',
                title = 'Position group in continent  %s in %s' % (origin, destination))
	...
	def update(attr, old, new):
		...
		if len(label_dict) == 0:
			p.title.text = 'Position group in Continent %s in %s' % (origin, destination)
	...
	origin_select = Select(title = 'Position Group', value = 'Attacked', options = origins)
